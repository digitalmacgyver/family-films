#!/usr/bin/env python
"""
Script to merge Irwin Wren (ID 156) into Irwin Wren (ID 98).
...
def merge_irwin_wren():
    """Merge person 156 into person 98."""
    
    try:
        person_98 = Person.objects.get(id=98)
        person_156 = Person.objects.get(id=156)
    except Person.DoesNotExist as e:
        print(f"Error: {e}")
        return
    
    print(f"Merging Irwin Wren duplicate:")
    print(f"  Target (keeping): {person_98.full_name()} (ID: 98)")
    print(f"  Duplicate (removing): {person_156.full_name()} (ID: 156)")
    print()
    
    with transaction.atomic():
        # Update FilmPeople relationships
        film_relationships = FilmPeople.objects.filter(person=person_156)
        film_count = film_relationships.count()
        print(f"Found {film_count} film relationships for person 156")
        ...
        chapter_relationships = ChapterPeople.objects.filter(person=person_156)
        chapter_count = chapter_relationships.count()
        print(f"\nFound {chapter_count} chapter relationships for person 156")
        ...
        # Delete person 156
        person_156.delete()
        print(f"\n✓ Deleted duplicate person (ID: 156)")
...
def verify_irwin_wren():
    ...
    # Check for Irwin Wren entries
    irwin_wrens = Person.objects.filter(first_name="Irwin", last_name="Wren")
    
    print(f"Found {irwin_wrens.count()} person(s) named 'Irwin Wren':")
